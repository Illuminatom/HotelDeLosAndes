...
public interface ProductosTiendaRepository extends JpaRepository<ProductosTienda, Integer> {
    ...
    @Query(value = "SELECT * FROM productos_tienda WHERE Tienda_id=:Tienda_id AND Producto_id=:Producto_id", nativeQuery = true)
    ProductosTienda darProductoTienda(@Param("Tienda_id") int Tienda_id, @Param("Producto_id") int Producto_id);

    @Modifying
    @Transactional
    @Query(value = "INSERT INTO productos_tienda (Tienda_id, Producto_id) VALUES (:Tienda_id, :Producto_id)", nativeQuery = true)
    void insertarProductoTienda(@Param("Tienda_id") int Tienda_id, @Param("Producto_id") int Producto_id);

    @Modifying
    @Transactional
    @Query(value = "UPDATE productos_tienda SET Tienda_id=:Tienda_id_actualizado, Producto_id=:Producto_id_actualizado WHERE Tienda_id=:Tienda_id AND Producto_id=:Producto_id", nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "DELETE FROM productos_tienda WHERE Tienda_id=:Tienda_id AND Producto_id=:Producto_id", nativeQuery = true)
