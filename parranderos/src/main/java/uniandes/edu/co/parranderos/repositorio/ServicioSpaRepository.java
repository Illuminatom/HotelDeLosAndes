...
public interface ServicioSpaRepository extends JpaRepository<ServicioSpa, Integer>{
    @Query(value = "SELECT * FROM serviciopa", nativeQuery = true)
    Collection<ServicioSpa> darServiciosSpas();

    @Query(value = "SELECT * FROM serviciospa WHERE id=:id", nativeQuery = true)
    ServicioSpa darServicioSpa(@Param("id") int id);

    @Modifying
    @Transactional
    @Query(value = "INSERT INTO serviciospa (id, duracion) VALUES (:id, :duracion)", nativeQuery = true)
    void insertarServicioSpa(@Param("id") int id, @Param("duracion") String duracion);

    @Modifying
    @Transactional
    @Query(value = "UPDATE serviciospa SET id=:id_actualizada, duracion=:duracion WHERE id=:id", nativeQuery = true)
    void actualizarServicioSpa(@Param("id") int id, @Param("id_actualizada") int id_actualizada, @Param("duracion") String duracion);

    @Modifying
    @Transactional
    @Query(value = "DELETE FROM serviciospa WHERE id=:id", nativeQuery = true)
